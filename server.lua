...
ePF = {}
Tunnel.bindInterface("epf_hpilegal",ePF)
...
local webhookcaminhao = "https://discordapp.com/api/webhooks/705987940005707817/7ZwqYiWsvh-sV4tN_HyNFnnGTddpPUkuy-cufBetuLoQWZ8OBIkL7olJppa7U5i812jY"
...
function ePF.money()
    ...
    local rola = math.random(10000, 15000)
    if user_id then
        if vRP.tryPayment(user_id,parseInt(rola)) then		    
		    TriggerClientEvent("Notify",source,"sucesso","Você pagou <b>$"..vRP.format(parseInt(rola)).."</b> dólares.")
            return true
        else
            TriggerClientEvent("Notify",source,"negado","Você nao tem <b>$"..vRP.format(parseInt(rola)).."</b> dólares aguarde um tempo para tentar novamente!.")
            return false
        end
	end
end
